...
public class MerchantWithdrawCardQueryService {
     ...
     * @throws Exception
     */
    public MerchantWithdrawCardQueryResponse execute(MerchantWithdrawCardQueryRequest request) throws TranFailException {
        logger.info("MerchantWithdrawCardQueryService.execute: request:{}", request);

        try {
            MerchantWithdrawCardQueryResponse response = HttpUtil.callHostServerByPost(false, null,
                    "UTF-8", request, "/acc/nb/nb_withdraw_card_qry",MerchantWithdrawCardQueryResponse.class);
